...
app/
  ...
  models/
    ...
    province_map.py      # ProvinceMap (map images for Province-type Locations — SEVERAL per province
                         #   allowed, distinguished by `label` e.g. North/South part) + ProvinceMapPlacement
                         #   (per-(map, child) overlay geometry in IMAGE-PIXEL coords JSONB; kind
                         #   point|line|region copied from LocationType.point_type at save)
  ...
  blueprints/
    main/views.py        # Public + admin-edit routes for Character / Faction / Role / Event / Location;
                         ...
                         #   renders the yearly territory-maps panel from chapter.date AND the
                         #   province-map sidebar panel via _chapter_province_maps — see feature #17);
                         #   faction leader add/remove;
  ...
  static/
    ...
    js/                  # chapter.js (refs click + link-style cookie + accordion-flash;
                         #     routes location-ref clicks to the province-map panel),
                         #   chapter_province_map.js (chapter sidebar's interactive province-map
                         #     panel: one tab per mentioned province, inner map switcher, draws
                         #     ProvinceMapPlacement overlays on the image_panzoom viewer, and
                         #     showLocation() → open Map accordion, activate tab+map, pan+pulse),
...
mcp_server/
  rotk_mcp.py            # Zero-dependency MCP server (stdio JSON-RPC, stdlib+requests — no SDK)
                         #   wrapping /api/v1 as read-only tools: rotk_api_index, rotk_list,
                         #   rotk_get, rotk_find_data_gaps (20+ data-quality sweeps), rotk_fetch.
                         #   Config: ROTK_API_BASE (default https://rotk.net). Wired into Claude
                         #   Code via repo-root .mcp.json. --selftest (offline) / --smoke (live).
...
16. **Public JSON API** (`/api/v1`, `app/blueprints/api/`). GET-only, anonymous, 120 req/min per IP. Lists return `{items, page, per_page, pages, total}` (per_page cap 100; chapters default 20 because full prose is included on lists AND detail — deliberate user decision). Payloads carry smart joins (character → factions/roles/sex-resolved relationships/portraits/urls/chapter keywords; event → sided faction lists with resolved labels; location → bulk-loaded ancestry chain; chapter → associated refs + prev/next). PRIVACY: no users/edits endpoints, `created_by`/`last_edited_by`/`notes` never serialized, private annotations never served, hidden/soft-deleted rows filtered everywhere — `tests/test_api.py::assert_no_private_keys` enforces this recursively. `registry.py` is the endpoint catalogue consumed by both the `GET /api/v1/` index and the `/admin/api-explorer` form generator — new endpoints must be added there too (province-maps landed this way: list + detail w/ placement geometry; location-types expose `point_type`). Chapters and year-maps are addressed by their public identifiers (`chapter_num`, `year`), not row ids. Read-only is STRUCTURAL: an `@api.before_request` guard 405s any non-GET method, and `tests/test_api.py::TestApiIsReadOnly` walks the URL map asserting no write methods exist. The `mcp_server/` directory wraps this API as MCP tools for Claude (also strictly read-only).
17. **Province maps** (`ProvinceMap`/`ProvinceMapPlacement`, admin-only for now). Uploaded images per Province-type Location — a province can carry SEVERAL labelled maps ("North part"/"South part"), each with independent placements (`/admin/province-maps`, YearMap-grade upload hardening, files `static/provincemaps/<location_id>_<map_id>.<ext>`; deleting a map cascades its placements). The editor (`/admin/province-maps/editor/<map_id>`, sibling-map switcher in the header) lists ALL descendants of the province (BFS over the parent hierarchy, with ancestry crumbs + aliases + a client-side type filter; saved shapes get click popups) and places each on the image according to its LocationType's `point_type`: 'point' = one click → FA-icon marker; 'line' = freehand stroke (map dragging disabled during the mode, stroke thinned to ~3px steps); 'region' = clicked-out polygon (≥3 vertices). Geometry is stored in IMAGE-PIXEL coordinates (`[x, y]` / `[[x, y], ...]` JSONB — CRS.Simple lat=y/lng=x conversion lives in the JS); `kind` is copied onto the placement at save so later point_type changes don't orphan rows. **Public rendering: the chapter-page sidebar Map panel** (`_chapter_province_maps` in `main/views.py` → `chapter_province_map.js`) walks each chapter location up to its Province ancestor, renders one tab per mentioned province that has a ProvinceMap (multi-map provinces get an inner switcher), and draws the placement overlays for the chapter's own locations on the reusable `image_panzoom` viewer. Clicking a location (inline `.location-ref` or a Locations-list row) opens the Map accordion, activates that location's province tab + map, and pans/pulses its pin; unplaced locations fall back to the Locations-accordion flash. This replaced the old lat/lng mini-map (`chapter_map.js`, removed; `map_base.js` still powers `/map`).
...
| `flask apply-fixes FILE [--apply]` | Generalized cross-resource bulk fixes from a JSON op list (`update` fields on any resource, `add/remove_relationship`, `add/update/remove_association`, `add/remove_faction_leader`). Dry-run by default; `--apply` writes (removals get a stronger confirm). Idempotent; audit + Edit log stamped; affected characters get mention recounts. Fix files live in `data/fixes/`. |
...
- Pure-function suites (no DB): `test_needle_pattern`,
  ...
  guarantees), `test_mcp_server` (network-free: monkeypatched HTTP). Cross-feature: `test_composites` (annotation vs
